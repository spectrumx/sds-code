...
```bash
docker compose -f compose.local.yaml run --rm django uv run python manage.py makemessages --all --no-location
```
...
```bash
docker compose -f compose.local.yaml run --rm django uv run python manage.py compilemessages
```
