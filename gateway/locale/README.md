...
```bash
# with the sds-gateway-local-app running:
docker exec -it sds-gateway-local-app uv run manage.py makemessages --all --no-location
```
...
```bash
# with the sds-gateway-local-app running:
docker exec -it sds-gateway-local-app uv run manage.py compilemessages
```
