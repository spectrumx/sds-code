...
```bash
docker compose -f compose.local.yaml run --rm django python manage.py makemessages --all --no-location
```
...
```bash
docker compose -f compose.local.yaml run --rm django python manage.py compilemessages
```
