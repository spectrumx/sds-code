...
function generate_certs() {
    ...
    openssl req -new -key admin-key.pem \
        -subj "/C=US/ST=INDIANA/L=SOUTH BEND/O=UNIVERSITY OF NOTRE DAME/OU=CRC/CN=admin" \
        -out admin.csr

    openssl x509 -req -in admin.csr -CA "${ROOT_CA_PEM}" -CAkey "${ROOT_CA_KEY_PEM}" \
    ...
    openssl pkcs8 -inform PEM -outform PEM -in opensearch-key-temp.pem -topk8 -nocrypt \
        -v1 PBE-SHA1-3DES -out opensearch-key.pem
    openssl req -new -key opensearch-key.pem \
        -subj "/C=US/ST=INDIANA/L=SOUTH BEND/O=UNIVERSITY OF NOTRE DAME/OU=CRC/CN=${OPEN_SEARCH_CONTAINER_NAME}" \
        -out opensearch.csr
    echo "subjectAltName=DNS:${OPEN_SEARCH_CONTAINER_NAME}" >opensearch.ext
    openssl x509 -req -in opensearch.csr -CA "${ROOT_CA_PEM}" -CAkey "${ROOT_CA_KEY_PEM}" \
        -CAcreateserial -sha256 -out opensearch.pem -days 730 -extfile opensearch.ext

    # Django cert
    openssl genrsa -out django-key-temp.pem 2048
    openssl pkcs8 -inform PEM -outform PEM -in django-key-temp.pem -topk8 -nocrypt \
        -v1 PBE-SHA1-3DES -out django-key.pem
    openssl req -new -key django-key.pem \
        -subj "/C=US/ST=INDIANA/L=SOUTH BEND/O=UNIVERSITY OF NOTRE DAME/OU=CRC/CN=${OPEN_SEARCH_CONTAINER_NAME}" \
        -out django.csr
    echo "subjectAltName=DNS:${OPEN_SEARCH_CONTAINER_NAME}" >django.ext
    openssl x509 -req -in django.csr -CA ${ROOT_CA_PEM} -CAkey ${ROOT_CA_KEY_PEM} \
        -CAcreateserial -sha256 -out django.pem -days 730 -extfile django.ext
...
function create_django_copy() {
    if [ -d "${DIR_HOST_CERTS_DJANGO}" ]; then
        echo "Removing existing django certs copy at '${DIR_HOST_CERTS_DJANGO}'"
        sudo rm -rf "${DIR_HOST_CERTS_DJANGO}"
    fi
...
function main() {
    ...
    pushd "${DIR_HOST_CERTS_TEMP}" || exit 1
    generate_certs
    cleanup_current_dir
    popd || exit 1
    replace_certs
