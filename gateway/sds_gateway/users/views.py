...
from sds_gateway.api_methods.utils.permissions import require_view_permission
from sds_gateway.api_methods.utils.permissions import require_edit_dataset_permission
from sds_gateway.api_methods.utils.permissions import can_user_access_item
from sds_gateway.api_methods.utils.permissions import get_user_permission_level
        ...
        try:
            ...
            if results["added"]:
                if len(results["added"]) == 1:
                    ...
                    messages.append(f"Added {', '.join(results['added'])}")
            if results["updated"]:
                messages.append(f"Updated {len(results['updated'])} permission(s)")
            if results["removed"]:
                if len(results["removed"]) == 1:
                    messages.append(f"Removed access for {results['removed'][0]}")
                else:
...
class ListDatasetsView(Auth0LoginRequiredMixin, View):
    ...
    def _get_shared_users_for_dataset(
        ...
        group_permissions: dict[str, dict] = {}

        for perm in shared_permissions:
            if perm.share_groups.exists():
                self._process_group_permission(perm, group_permissions, current_user)
            else:
