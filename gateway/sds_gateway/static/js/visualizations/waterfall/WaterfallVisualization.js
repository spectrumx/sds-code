...
class WaterfallVisualization {
    ...
    async initialize() {
        try {
            ...
            console.error(
                "Failed to initialize waterfall visualization:",
                error,
            )
    ...
    initializeComponents() {
        ...
        this.controls = new WaterfallControls(
            (currentSliceIndex, waterfallWindowStart) => {
                ...
                this.waterfallRenderer.setWaterfallWindowStart(
                    waterfallWindowStart,
                )
                ...
                if (sliceChanged) {
                    ...
                    const sliceInCache =
                        this.sliceCache?.getSlice(currentSliceIndex)
    ...
    async render() {
        if (this.isStreamingMode) {
            ...
            if (
                ...
                this.parsedWaterfallData.length === 0
            ) {
                return
            }
        }
            ...
            if (slice?.data) {
                ...
                try {
                    ...
                    if (this.currentSliceIndex === targetSliceIndex) {
                        const loadedSlice =
                            this.sliceCache.getSlice(targetSliceIndex)
    ...
    async renderWaterfall() {
        if (this.isStreamingMode) {
            ...
            const missing = this.sliceCache.getMissingSlices(
                startIndex,
                endIndex,
            )
            if (missing.length > 0) {
    ...
    async loadWaterfallData() {
        try {
                ...
                try {
                    ...
                    if (
                        ...
                        const streamingSuccess =
                            await this.tryLoadStreamingMode()
            ...
            if (
                ...
                userMessage =
                    "You do not have permission to access this capture."
    ...
    async fetchWaterfallResult() {
        try {
            ...
            this.controls.setTotalSlices(this.totalSlices)
            this.waterfallRenderer.setScaleBounds(this.scaleMin, this.scaleMax)
            this.periodogramChart.updateYAxisBounds(
                this.scaleMin,
                this.scaleMax,
            )
            this.updateColorLegend()
