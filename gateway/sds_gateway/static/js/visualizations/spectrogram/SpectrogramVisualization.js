...
export class SpectrogramVisualization {
    ...
    async initialize() {
        try {
            ...
            console.error(
                "Failed to initialize spectrogram visualization:",
                error,
            )
    ...
    async createSpectrogramJob(settings) {
        try {
            ...
            if (!data.uuid) {
                const missingUuidError = new Error(
                    "Spectrogram job ID not found",
                )
                missingUuidError.userMessage =
                    "Unable to create spectrogram job."
                missingUuidError.errorDetail =
    ...
    async checkJobStatus() {
        ...
        try {
            ...
            this.displayRequestError(
                error,
                "Failed to check spectrogram status",
            )
            this.setGeneratingState(false)
        }
    }
    ...
    async fetchSpectrogramResult() {
        try {
            ...
            this.displayRequestError(
                error,
                "Failed to fetch spectrogram result",
            )
            this.setGeneratingState(false)
        }
    }
