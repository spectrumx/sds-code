	...
	static initializeModal(config = {}) {
		...
		if (permissions && managersOut && window.PermissionsManager) {
			...
			if (wire === "dataset" || wire === "both") {
				ModalManager._wireDatasetListModalsInternal(permMgr, managersOut);
			}
			if (wire === "capture" || wire === "both") {
				ModalManager._wireCaptureListModalsInternal(permMgr, managersOut);
			}
		}
		...
		if (
			...
			const permissionsManager = new window.PermissionsManager(permissions);
			const uuidRegex =
