 ...
 * Migrated from file_list_upload_capture_modal.js (file list page flow).
 */
class UploadCaptureModalController {
	...
	async handleSkippedFilesUpload(allRelativePaths, abortController) {
		...
		window.ChunkUploadPipeline.appendCaptureTypeToFormData(skippedFormData);
	...
	calculateTotalChunks(filesToUpload, chunkSizeBytes) {
		return window.ChunkUploadPipeline.calculateTotalChunks(
			filesToUpload,
			chunkSizeBytes,
		);
	}
	...
	async uploadChunk({
		...
		window.ChunkUploadPipeline.appendCaptureTypeToFormData(chunkFormData);
	...
	showUploadResults(result, uploadedCount, totalCount, skippedCount = 0) {
		...
		if (!modalBody || !resultModalEl || !window.bootstrap?.Modal) {
			return;
		}
		const modal = new bootstrap.Modal(resultModalEl);

		const uploadModalInstance = bootstrap.Modal.getInstance(
