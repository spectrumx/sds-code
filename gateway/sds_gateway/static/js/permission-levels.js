...
function initializeExistingUserPermissionSelectors() {
	...
	for (const select of existingUserSelectors) {
...
function initializePermissionBadges() {
	...
	for (const badge of permissionBadges) {
		// Add hover tooltip
...
function initializeGroupPermissionManagement() {
	...
	for (const select of groupPermissionSelectors) {
	...
	for (const select of memberPermissionSelectors) {
	...
	fetch(`/users/share/${itemType}/${itemUuid}/`, {
		...
		.then((data) => {
			if (data.success) {
				...
				for (const memberSelect of memberSelectors) {
