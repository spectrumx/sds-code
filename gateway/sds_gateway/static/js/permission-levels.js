...
function initializeExistingUserPermissionSelectors() {
    ...
    const existingUserSelectors = document.querySelectorAll('.permission-level-selector');
...
function updatePermissionSelectorAppearance(select, permissionLevel) {
    ...
    select.classList.remove('permission-viewer', 'permission-contributor', 'permission-co-owner');
...
function initializeGroupPermissionManagement() {
    ...
    const groupPermissionSelectors = document.querySelectorAll('.group-permission-selector');
    ...
    const memberPermissionSelectors = document.querySelectorAll('.member-permission-selector');
...
function initializeMemberPermissionToggles() {
    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', function() {
            const memberPermissions = this.parentElement.querySelector('.member-permissions');
            const icon = this.querySelector('i');
            
            if (memberPermissions.style.display === 'none') {
                memberPermissions.style.display = 'block';
                icon.className = 'bi bi-chevron-up';
                this.innerHTML = '<i class="bi bi-chevron-up"></i> Hide Individual Permissions';
            } else {
                memberPermissions.style.display = 'none';
                icon.className = 'bi bi-chevron-down';
                this.innerHTML = '<i class="bi bi-chevron-down"></i> Manage Individual Permissions';
            }
        });
    });
}
...
function handleGroupPermissionChange(event) {
    ...
    .then(data => {
        if (data.success) {
            ...
            const memberSelectors = document.querySelectorAll(`[data-group-uuid="${groupUuid}"].member-permission-selector`);
            
            memberSelectors.forEach(memberSelect => {
                memberSelect.value = newPermissionLevel;
                memberSelect.classList.remove('permission-viewer', 'permission-contributor', 'permission-co-owner');
            ...
            // Show error message
            showAlert(data.error || 'Failed to update group permission level', 'error');
...
function handleMemberPermissionChange(event) {
    ...
    .then(data => {
        if (data.success) {
            // Show success message
            showAlert(`Updated ${memberEmail} to ${newPermissionLevel} permission`, 'success');
            
            // Check if all members have the same permission level
            const memberSelectors = document.querySelectorAll(`[data-group-uuid="${groupUuid}"].member-permission-selector`);
            const allSame = Array.from(memberSelectors).every(memberSelect => memberSelect.value === newPermissionLevel);
            
            // Update group selector
            const groupSelector = document.querySelector(`[data-group-uuid="${groupUuid}"].group-permission-selector`);
            if (groupSelector) {
                if (allSame) {
                    groupSelector.value = newPermissionLevel;
                    updatePermissionSelectorAppearance(groupSelector, newPermissionLevel);
                } else {
            ...
            // Show error message
            showAlert(data.error || 'Failed to update member permission level', 'error');
