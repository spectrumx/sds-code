...
describe("DatasetCreationHandler", () => {
    ...
    describe("Asset Selection", () => {
        ...
        test("should update hidden fields with current selections", () => {
            ...
            expect(creationHandler.selectedCapturesField.value).toContain(
                "capture1",
            )
            expect(creationHandler.selectedCapturesField.value).toContain(
                "capture2",
    ...
    describe("Form Submission", () => {
        test.each([
            ...
            (
                ...
                expect(creationHandler.submitBtn.disabled).toBe(
                    expectedDisabled,
                )
                if (loading) {
                    expect(creationHandler.submitBtn.innerHTML).toContain(
                        expectedText,
                    )
                } else {
                    expect(creationHandler.submitBtn.textContent).toBe(
                        expectedText,
                    )
                }
            },
    ...
    describe("Search Handler Integration", () => {
        ...
        test("should handle capture search results", () => {
            ...
            expect(creationHandler.capturesSearchHandler).toBe(
                mockSearchHandler,
            )
        })
    })
