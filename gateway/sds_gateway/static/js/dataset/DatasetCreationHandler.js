...
class DatasetCreationHandler extends BaseManager {
    ...
    constructor(config) {
        ...
        // Hidden fields
        this.selectedCapturesField =
            document.getElementById("selected_captures")
        ...
        if (this.nameField && nameDisplays.length > 0) {
            for (const nameDisplay of Array.from(nameDisplays)) {
                nameDisplay.textContent =
                    this.nameField.value || "Untitled Dataset"
            }
        }
    }
    ...
    updateHiddenFields() {
        ...
        if (this.selectedCapturesField) {
            this.selectedCapturesField.value = Array.from(
