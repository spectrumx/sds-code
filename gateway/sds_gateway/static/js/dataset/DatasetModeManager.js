...
class DatasetModeManager {
	...
	captureOriginalDatasetData() {
		...
		if (this.isEditMode) {
			...
			if (nameField) {
				...
				originalData.name = readonlyNameField?.value || this.config.existingDatasetName || 'Untitled Dataset';
			}
			...
			if (statusField) {
				...
				originalData.status = readonlyStatusField?.value || this.config.existingDatasetStatus || 'Unknown';
			}
			
			if (descriptionField) {
				originalData.description = descriptionField.value || 'No description provided.';
			} else {
				// Fall back to readonly field or config data
				const readonlyDescField = document.querySelector('textarea[readonly]');
				originalData.description = readonlyDescField?.value || this.config.existingDatasetDescription || 'No description provided.';
			}
	...
	initializeCreateModeUI() {
		...
		const submitBtn = document.getElementById("submitForm");
		if (submitBtn && this.handler.currentStep !== this.handler.steps.length - 1) {
	...
	initializeStepNavigation() {
		...
		this.steps = [
			...
			{ id: "step4", name: this.isEditMode ? "Review and Update" : "Review and Create" }
		];
	...
	updateStepDisplay() {
		...
		// Update step content
		const stepPanes = document.querySelectorAll("#stepTabsContentInner .tab-pane");
		...
		if (this.submitBtn) {
			this.submitBtn.style.display = this.currentStep === this.steps.length - 1 ? "inline-block" : "none";
		}
	}
	...
	handleSubmit(e) {
		// Delegate to handler
		if (this.handler && typeof this.handler.handleSubmit === 'function') {
	...
	updateDatasetName(nameField) {
		...
		if (this.isEditMode) {
			...
			if (currentValue !== originalValue) {
				const changesList = nameElement.querySelector('.pending-changes') || document.createElement('ul');
	...
	updateDatasetStatus(statusField) {
		...
		if (this.isEditMode) {
			...
			// Add pending changes if different from original
			if (currentValue !== originalValue) {
				const changesList = statusElement.querySelector('.pending-changes') || document.createElement('ul');
				changesList.className = 'pending-changes list-unstyled mt-2';
				changesList.innerHTML = `
					<li class="text-warning">
						<i class="bi bi-pencil me-1"></i>
	...
	updateDatasetDescription(descriptionField) {
		...
		if (this.isEditMode) {
			...
			// Add pending changes if different from original
			if (currentValue !== originalValue) {
				const changesList = descriptionElement.querySelector('.pending-changes') || document.createElement('ul');
				changesList.className = 'pending-changes list-unstyled mt-2';
				changesList.innerHTML = `
					<li class="text-warning">
						<i class="bi bi-pencil me-1"></i>
				...
				// Remove pending changes if no changes
				const changesList = descriptionElement.querySelector('.pending-changes');
				if (changesList) {
					changesList.remove();
				}
			}
	...
	updateDatasetAuthors(authorsField) {
		...
		if (window.updateDatasetAuthors && typeof window.updateDatasetAuthors === 'function') {
	...
	setupAuthorManagementListeners() {
		...
		document.addEventListener('input', (e) => {
			if (e.target.classList.contains('author-name-input') || 
		...
		document.addEventListener('click', (e) => {
			if (e.target.id === 'add-author-btn' || e.target.closest('#add-author-btn')) {
	...
	updatePendingChanges() {
		...
		if (totalChanges === 0) {
			pendingTable.innerHTML = '<tr><td colspan="5" class="text-center text-muted">No pending asset changes</td></tr>';
		}
	}
	...
	loadFileTree() {
		...
		if (this.filesSearchHandler && typeof this.filesSearchHandler.loadFileTree === 'function') {
	...
	updateSelectedAssets() {
		...
		if (this.handler && typeof this.handler.updateSelectedCapturesTable === 'function') {
		...
		if (this.handler && typeof this.handler.updateSelectedFilesTable === 'function') {
		...
		if (capturesCount) {
			const count = this.handler && this.handler.selectedCaptures ? this.handler.selectedCaptures.size : 0;
			capturesCount.textContent = `${count} selected`;
		}
		if (filesCount) {
			const count = this.handler && this.handler.selectedFiles ? this.handler.selectedFiles.size : 0;
	...
	updateUIForPermissions() {
		...
		const permissionDisplay = document.getElementById("user-permission-display");
	...
	updateFormElementsForPermissions() {
		...
		if (!this.permissions.canEditMetadata()) {
			[nameField, authorsField, statusField, descriptionField].forEach(field => {
				if (field) {
					field.disabled = true;
