...
function initializeAddMembersForm() {
    ...
    if (addMembersForm) {
        addMembersForm.addEventListener('submit', function(e) {
            ...
            const selectedUsers = shareGroupUserSearchHandler ? (shareGroupUserSearchHandler.selectedUsersMap[inputId] || []) : [];
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.added_users && data.added_users.length > 0) {
                        updateTableMemberEmails(currentGroupUuid, data.added_users, 'add');
                    }
                } else {
                    showAlert(data.error, 'error');
                }
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.removed_users && data.removed_users.length > 0) {
                            updateTableMemberEmails(currentGroupUuid, data.removed_users, 'remove');
                        }
                    } else {
                        showAlert(data.error, 'error');
                    }
    ...
    .then(data => {
        if (data.success) {
            if (data.members.length === 0) {
                ...
                const memberHtml = data.members.map(member => `
                    <tr>
                        <td class="p-2 shadow-sm">
                            <div class="row">
                                <div class="col-md-10">
...
function initializeShareGroupUserSearch() {
    ...
    shareGroupUserSearchHandler.resetShareGroup = function() {
        ...
        if (input) {
            input.value = '';
            const chipContainer = input.closest('.user-search-input-container').querySelector('.selected-users-chips');
            if (chipContainer) {
                chipContainer.innerHTML = '';
            }
        }
...
function updateSaveButtonState() {
    const saveBtn = document.getElementById('save-sharegroup-btn');
    const pendingMessage = document.getElementById('pending-changes-message-sharegroup');
    const pendingMessageFooter = document.getElementById('pending-changes-message-sharegroup-footer');
    
    const hasSelectedUsers = shareGroupUserSearchHandler && 
        shareGroupUserSearchHandler.selectedUsersMap['user-search-sharegroup'] && 
        shareGroupUserSearchHandler.selectedUsersMap['user-search-sharegroup'].length > 0;
...
function updateTableMemberInfo(groupUuid, members) {
    const memberCountElement = document.querySelector(`.member-count[data-group-uuid="${groupUuid}"]`);
    const memberEmailsElement = document.querySelector(`.member-emails[data-group-uuid="${groupUuid}"]`);
...
function updateTableMemberEmails(groupUuid, emails, action) {
    ...
    const currentCount = parseInt(currentCountText.match(/(\d+)/)[1]);
    ...
    if (memberEmailsElement.style.display !== 'none') {
        ...
        const emailsMatch = emailsText.match(/([^,]+(?:,[^,]+)*?)(?:\s+and\s+\d+\s+more)?$/);
        if (emailsMatch) {
