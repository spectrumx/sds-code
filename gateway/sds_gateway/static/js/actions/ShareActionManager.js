...
class ShareActionManager extends ModalManager {
    ...
    setupModalEventHandlers() {
        ...
        if (!modal) {
            console.error(
                `Modal not found for ${this.itemType}: ${this.itemUuid}`,
            )
            return
        }
    ...
    _commitViewerSelection(input, userEmail, userName) {
        if (
            !this.selectedUsersMap[input.id].some((u) => u.email === userEmail)
        ) {
    ...
    async handleShareItem() {
        ...
        if (this.pendingRemovals.size > 0) {
            formData.remove_users = JSON.stringify(
                Array.from(this.pendingRemovals),
            )
        }
    ...
    selectUser(item, input) {
        ...
        if (
            !this.selectedUsersMap[inputId].some((u) => u.email === userEmail)
        ) {
