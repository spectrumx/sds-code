...
class CaptureReindexingManager extends ModalManager {
	constructor() {
		...
		this.pendingSectionEl = document.getElementById("reindex-pending-section");
	...
	initializeEventListeners() {
		...
		if (this.confirmBtn) {
			this.confirmBtn.addEventListener("click", () => void this.confirmReindex());
		}
	}
	...
	async setPreviewLoading(visible) {
		...
		await window.DOMUtils.renderLoading(this.loadingEl, "Checking for new or updated files…", {
			format: "block",
			size: "sm",
		});
	}
	...
	renderPendingList(pendingFiles) {
		...
		const formatSize = window.DOMUtils?.formatFileSize?.bind(window.DOMUtils);
		for (const row of pendingFiles) {
			...
			statusTd.textContent = row.status === "updated" ? "Updated" : "Not linked";
