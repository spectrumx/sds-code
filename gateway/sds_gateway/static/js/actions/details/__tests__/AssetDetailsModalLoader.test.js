...
const { createMockFetchResponse } = require("../../../tests-config/testHelpers.js");
...
describe("AssetDetailsModalLoader", () => {
	...
	describe("findDelegateTarget", () => {
		test("returns element when it matches selector", () => {
			...
			expect(
				AssetDetailsModalLoader.findDelegateTarget(el, [".details-trigger"]),
			).toBe(el);
		});
		...
		test("returns closest ancestor match", () => {
			...
			expect(
				AssetDetailsModalLoader.findDelegateTarget(child, [".capture-link"]),
			).toBe(parent);
		});
	...
	describe("ensureDetailsClickDelegation", () => {
		test("wires click once and cleanup removes listener", () => {
			...
			const cleanup = AssetDetailsModalLoader.ensureDetailsClickDelegation();
			expect(document.body.dataset.detailsAssetClickWired).toBe("1");

			const cleanup2 = AssetDetailsModalLoader.ensureDetailsClickDelegation();
		...
		test("delegated capture click prevents default and opens details", async () => {
			...
			window.DetailsModalAssetRegistry = {
				capture: {
					...
					resolveUuidFromTrigger: (el) => el.getAttribute("data-item-uuid"),
				},
			};
			const cleanup =
				AssetDetailsModalLoader.attachDocumentDetailsClickDelegation();
					...
					new Promise((resolve) => {
						resolveFetch = () =>
							resolve({
								ok: true,
								json: () =>
									Promise.resolve({ title: "T", html: "", meta: {} }),
							});
					}),
			);
			const pending =
