...
global.APIClient = {
	...
	get: async (url, params) => {
		...
		return [
			...
			{ name: "Test Group", email: "group:test-uuid", type: "group", member_count: 5 }
		];
	}
};
...
class ShareActionManagerTests {
	...
	createMockShareActionManager(config = {}) {
		...
		return {
			...
			searchUsers: async function(query, dropdown) {
				// Mock implementation
				return [];
			},
			
			selectUser: function(item, input) {
				// Mock implementation
			},
			
			renderChips: function(input) {
				// Mock implementation
			},
...
sharingTests.addTest("Owner can share items", () => {
	...
	sharingTests.assert(permissions.canShare(), "Owner should be able to share items");
});
...
sharingTests.addTest("Co-owner can share items", () => {
	...
	sharingTests.assert(permissions.canShare(), "Co-owner should be able to share items");
});
...
sharingTests.addTest("Contributor cannot share items", () => {
	...
	sharingTests.assert(!permissions.canShare(), "Contributor should not be able to share items");
});
...
sharingTests.addTest("Viewer cannot share items", () => {
	...
	sharingTests.assert(!permissions.canShare(), "Viewer should not be able to share items");
});
...
sharingTests.addTest("Group selection works correctly", () => {
	...
	sharingTests.assertEqual(manager.selectedUsersMap[inputId][0].member_count, 5);
});
...
sharingTests.addTest("Clear selections works correctly", () => {
	...
	manager.pendingPermissionChanges.set("change@example.com", { permissionLevel: "contributor" });
...
sharingTests.addTest("Search users works correctly", async () => {
	...
	sharingTests.assert(Array.isArray(results), "Search results should be an array");
});
...
sharingTests.addTest("User chip creation works correctly", () => {
	...
	sharingTests.assert(chip.includes("user-chip"), "Chip should have user-chip class");
	sharingTests.assert(chip.includes("test@example.com"), "Chip should contain user email");
});
...
sharingTests.addTest("Group chip creation works correctly", () => {
	...
	sharingTests.assert(chip.includes("user-chip"), "Chip should have user-chip class");
	sharingTests.assert(chip.includes("Test Group"), "Chip should contain group name");
});
...
sharingTests.addTest("Permission level validation works correctly", () => {
	...
	validLevels.forEach(level => {
		sharingTests.assert(validLevels.includes(level), `Level ${level} should be valid`);
	});
	
	invalidLevels.forEach(level => {
		sharingTests.assert(!validLevels.includes(level), `Level ${level} should be invalid`);
	});
});
...
sharingTests.addTest("Multiple user selection works correctly", () => {
	...
	// Add multiple users
	manager.selectedUsersMap[inputId] = [
		{ name: "User 1", email: "user1@example.com", type: "user", permission_level: "viewer" },
		{ name: "User 2", email: "user2@example.com", type: "user", permission_level: "contributor" },
		{ name: "Group 1", email: "group:group1-uuid", type: "group", member_count: 3, permission_level: "co-owner" }
	];
	
	sharingTests.assertEqual(manager.selectedUsersMap[inputId].length, 3);
	
	// Check individual users
	const user1 = manager.selectedUsersMap[inputId].find(u => u.email === "user1@example.com");
	const user2 = manager.selectedUsersMap[inputId].find(u => u.email === "user2@example.com");
	const group1 = manager.selectedUsersMap[inputId].find(u => u.email === "group:group1-uuid");
...
sharingTests.addTest("Duplicate user prevention works correctly", () => {
	...
	// Add user
	manager.selectedUsersMap[inputId] = [
		{ name: "Test User", email: "test@example.com", type: "user", permission_level: "viewer" }
	];
	
	// Try to add same user again
	const existingUser = manager.selectedUsersMap[inputId].find(u => u.email === "test@example.com");
...
sharingTests.addTest("Mixed permission changes and removals work correctly", () => {
	const manager = sharingTests.createMockShareActionManager();
	...
	sharingTests.assert(manager.pendingPermissionChanges.has("change@example.com"));
	sharingTests.assert(manager.pendingRemovals.has("remove@example.com"));
	sharingTests.assert(!manager.pendingPermissionChanges.has("remove@example.com"));
...
sharingTests.addTest("Share with notification works correctly", () => {
	...
	sharingTests.assert(formData.notify_users === "1", "Notification should be enabled");
	sharingTests.assert(formData.notify_message.length > 0, "Notification message should be present");
});
