...
class MockPermissionsManager {
	...
	canEdit() {
		return (
			this.permissions.capturePermissions?.canEditMetadata ||
...
class MockDetailsActionManager {
	...
	async handleCaptureDetails(uuid) {
		...
		const data = await APIClient.get(
			`/users/capture-details/?capture_uuid=${uuid}`,
		);
...
const DetailsActionManager =
	global.DetailsActionManager || MockDetailsActionManager;
...
class DetailsFunctionalityTests {
	...
	setupTests() {
		...
		this.addTest(
			"DetailsActionManager should initialize with permissions",
			() => {
		...
		this.addTest(
			"ModalManager should initialize with modal configuration",
			() => {
		...
		this.addTest(
			"ModalManager should show modal with title and content",
			() => {
				const modalManager = new ModalManager({
					modalId: "capture-modal",
					modalBodyId: "capture-modal-body",
		...
		this.addTest("Should update capture name via API", async () => {
			...
			const result = await modalManager.updateCaptureName(
				"test-uuid",
				"New Name",
			);
		...
		this.addTest("Should handle modal errors gracefully", async () => {
			...
			global.document.getElementById = (id) => {
				if (id === "files-section-placeholder") {
					return {
						get innerHTML() {
							return filesSectionContent;
						},
						set innerHTML(value) {
							filesSectionContent = value;
						},
					};
				}
				return mockDOM.createElement("div");
			...
			if (!filesSectionContent.includes("Error loading")) {
				throw new Error(
					`Should show error message, got: "${filesSectionContent}"`,
				);
			}
		});
