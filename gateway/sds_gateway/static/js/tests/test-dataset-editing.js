...
global.APIClient = {
	...
	get: async (url, params) => {
		...
		return {
			captures: [
				{ id: 1, type: "spectrum", directory: "/test", owner_name: "Test User" }
			],
			files: [
				{ id: 1, name: "test.h5", media_type: "application/x-hdf", relative_path: "/test.h5", size: "1.2 MB" }
			]
		};
	}
};
...
class PermissionsManager {
	...
	canEditMetadata() {
		return this.isOwner || ["co-owner", "contributor"].includes(this.userPermissionLevel);
	}
	...
	canAddAsset(asset) {
		...
		return asset.owner_id === this.currentUserId && this.userPermissionLevel === "contributor";
	}
...
datasetEditingTests.addTest("Dataset creation handler initializes correctly", () => {
...
datasetEditingTests.addTest("Dataset editing handler initializes correctly", () => {
...
datasetEditingTests.addTest("Step navigation works correctly in creation mode", () => {
	...
	datasetEditingTests.assert(!invalidForward, "Should not navigate beyond steps");
	
	const invalidBackward = handler.navigateStep(-10);
	datasetEditingTests.assert(!invalidBackward, "Should not navigate below zero");
});

// Test 4: Step validation in creation mode
datasetEditingTests.addTest("Step validation works correctly in creation mode", () => {
	const handler = datasetEditingTests.createMockDatasetCreationHandler();
	
	// Test valid steps
	handler.currentStep = 0;
	datasetEditingTests.assert(handler.validateCurrentStep(), "Step 0 should be valid");
	
	handler.currentStep = 1;
	datasetEditingTests.assert(handler.validateCurrentStep(), "Step 1 should be valid");
	
	// Test invalid step
	handler.currentStep = 10;
	datasetEditingTests.assert(!handler.validateCurrentStep(), "Step 10 should be invalid");
});

// Test 5: Capture selection in creation mode
datasetEditingTests.addTest("Capture selection works correctly in creation mode", () => {
	const handler = datasetEditingTests.createMockDatasetCreationHandler();
...
datasetEditingTests.addTest("File selection works correctly in creation mode", () => {
	const handler = datasetEditingTests.createMockDatasetCreationHandler();
...
datasetEditingTests.addTest("Capture removal works correctly in editing mode", () => {
	const handler = datasetEditingTests.createMockDatasetEditingHandler();
...
datasetEditingTests.addTest("File removal works correctly in editing mode", () => {
	const handler = datasetEditingTests.createMockDatasetEditingHandler();
...
datasetEditingTests.addTest("Capture addition works correctly in editing mode", () => {
	const handler = datasetEditingTests.createMockDatasetEditingHandler();
	
	// Add capture to pending
	const captureData = { id: "new-capture", type: "spectrum", directory: "/new" };
...
datasetEditingTests.addTest("File addition works correctly in editing mode", () => {
	const handler = datasetEditingTests.createMockDatasetEditingHandler();
...
datasetEditingTests.addTest("Permission-based access control works correctly", () => {
	...
	datasetEditingTests.assert(ownerPermissions.canEditMetadata(), "Owner should be able to edit metadata");
	datasetEditingTests.assert(ownerPermissions.canAddAssets(), "Owner should be able to add assets");
	datasetEditingTests.assert(ownerPermissions.canRemoveAnyAssets(), "Owner should be able to remove any assets");
	datasetEditingTests.assert(ownerPermissions.canRemoveOwnAssets(), "Owner should be able to remove their own assets");
	...
	datasetEditingTests.assert(contributorPermissions.canEditMetadata(), "Contributor should be able to edit metadata");
	datasetEditingTests.assert(contributorPermissions.canAddAssets(), "Contributor should be able to add assets");
	datasetEditingTests.assert(!contributorPermissions.canRemoveAnyAssets(), "Contributor should not be able to remove any assets");
	datasetEditingTests.assert(contributorPermissions.canRemoveOwnAssets(), "Contributor should be able to remove their own assets");
	...
	datasetEditingTests.assert(!viewerPermissions.canEditMetadata(), "Viewer should not be able to edit metadata");
	datasetEditingTests.assert(!viewerPermissions.canAddAssets(), "Viewer should not be able to add assets");
	datasetEditingTests.assert(!viewerPermissions.canRemoveAnyAssets(), "Viewer should not be able to remove any assets");
	datasetEditingTests.assert(!viewerPermissions.canRemoveOwnAssets(), "Viewer should not be able to remove their own assets");
});

// Test 14: Asset ownership validation
datasetEditingTests.addTest("Asset ownership validation works correctly", () => {
	...
	datasetEditingTests.assert(permissions.canAddAsset(ownedAsset), "Should be able to add owned asset");
	datasetEditingTests.assert(!permissions.canAddAsset(otherAsset), "Should not be able to add other's asset");
	
	// Test asset removal permissions
	datasetEditingTests.assert(!permissions.canRemoveAsset(ownedAsset), "Contributor should not be able to remove assets");
	datasetEditingTests.assert(!permissions.canRemoveAsset(otherAsset), "Should not be able to remove other's asset");
});

// Test 15: Mixed operations in editing mode
datasetEditingTests.addTest("Mixed operations work correctly in editing mode", () => {
	const handler = datasetEditingTests.createMockDatasetEditingHandler();
...
datasetEditingTests.addTest("Error handling works correctly in form submission", async () => {
	const handler = datasetEditingTests.createMockDatasetCreationHandler();
	...
	try {
		...
		datasetEditingTests.assert(error.message === "Network error", "Should catch the network error");
	} finally {
...
datasetEditingTests.addTest("Hidden field updates work correctly", () => {
	...
	datasetEditingTests.assert(typeof handler.updateHiddenFields === "function", "updateHiddenFields should be a function");
});
...
datasetEditingTests.addTest("Step change callbacks work correctly", () => {
	...
	datasetEditingTests.assert(typeof handler.onStepChange === "function", "onStepChange should be a function");
});

// Test 19: Dataset mode detection
datasetEditingTests.addTest("Dataset mode detection works correctly", () => {
	// Test creation mode
	const creationHandler = datasetEditingTests.createMockDatasetCreationHandler();
	datasetEditingTests.assert(!creationHandler.datasetUuid, "Creation handler should not have datasetUuid");
	
	// Test editing mode
	const editingHandler = datasetEditingTests.createMockDatasetEditingHandler({
		datasetUuid: "test-uuid"
	});
	datasetEditingTests.assert(editingHandler.datasetUuid, "Editing handler should have datasetUuid");
});

// Test 20: Complex workflow simulation
datasetEditingTests.addTest("Complex workflow simulation works correctly", () => {
	const handler = datasetEditingTests.createMockDatasetEditingHandler();
	...
	// 2. Add new capture
	handler.addCaptureToPending("new-capture", { type: "spectrum", directory: "/new" });
	...
	const removeCapture = changes.captures.find(([id, change]) => change.action === "remove");
	const addCapture = changes.captures.find(([id, change]) => change.action === "add");
	const removeFile = changes.files.find(([id, change]) => change.action === "remove");
	const addFile = changes.files.find(([id, change]) => change.action === "add");
	
	datasetEditingTests.assert(removeCapture !== undefined, "Should have remove capture operation");
	datasetEditingTests.assert(addCapture !== undefined, "Should have add capture operation");
	datasetEditingTests.assert(removeFile !== undefined, "Should have remove file operation");
	datasetEditingTests.assert(addFile !== undefined, "Should have add file operation");
});
