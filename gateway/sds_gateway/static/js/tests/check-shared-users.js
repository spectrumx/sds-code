...
function getSharedUsers(modalId) {
    ...
    const usersWithAccessSection = modal.querySelector(`#users-with-access-section-${modalId.replace('share-modal-', '')}`);
    ...
    userRows.forEach((row, index) => {
        ...
        const permissionElement = row.querySelector('.badge, .access-level-dropdown');
        ...
        if (nameElement && emailElement) {
            const user = {
                ...
                permission: permissionElement ? permissionElement.textContent.trim() : 'Unknown',
                isOwner: index === 0,
...
function getAllSharedUsers() {
    const modals = document.querySelectorAll('.modal[data-item-uuid][data-item-type]');
    const result = {};
...
function compareSharedUsers(before, after) {
    ...
    Object.keys(before).forEach(modalId => {
        ...
        beforeUsers.forEach(beforeUser => {
...
function logSharedUsers(users, title = 'Shared Users') {
    ...
    Object.keys(users).forEach(modalId => {
        ...
        modal.sharedUsers.forEach((user, index) => {
            const icon = user.isOwner ? '👑' : user.type === 'group' ? '👥' : '👤';
            console.log(`    ${index + 1}. ${icon} ${user.name} (${user.email}) - ${user.permission}`);
        });
    });
}
...
function logChanges(changes) {
    ...
    if (changes.unchanged.length > 0) {
        ...
        changes.unchanged.forEach(modalId => console.log(`   - ${modalId}`));
    }
    
    if (changes.added.length > 0) {
        console.log(`➕ Users added: ${changes.added.length}`);
        changes.added.forEach(change => {
            console.log(`   - ${change.modalId}: ${change.user.name} (${change.user.email})`);
        });
    }
    ...
    if (changes.modified.length > 0) {
        ...
        changes.modified.forEach(change => {
            console.log(`   - ${change.modalId}: ${change.before.name} (${change.before.email})`);
...
// Quick test function
window.testShareModalRefresh = async function(itemType = 'dataset') {
    ...
    const modals = document.querySelectorAll(`.modal[data-item-type="${itemType}"]`);
    ...
    if (!shareManager) {
        console.error('❌ No ShareActionManager found. Make sure a share modal is loaded.');
        return;
    }
...
console.log('  getSharedUsers("share-modal-uuid") - Get users from specific modal');
