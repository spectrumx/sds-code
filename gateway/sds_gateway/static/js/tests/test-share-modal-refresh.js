...
class ShareModalRefreshTest {
    ...
    findShareActionManager(itemType) {
        ...
        if (window.ShareActionManager) {
            ...
            console.warn('Found ShareActionManager class but no instance. You may need to manually trigger refresh.');
        }
    ...
    logResults(comparison) {
        ...
        if (comparison.unchanged.length > 0) {
            ...
            comparison.unchanged.forEach(modalId => {
                console.log(`   - ${modalId}`);
            });
        }
        ...
        if (comparison.usersAdded.length > 0) {
            ...
            comparison.usersAdded.forEach(change => {
        ...
        if (comparison.usersRemoved.length > 0) {
            ...
            comparison.usersRemoved.forEach(change => {
                console.log(`   - ${change.modalId}: ${change.user.name} (${change.user.email})`);
            });
        }
        ...
        if (comparison.usersModified.length > 0) {
            ...
            comparison.usersModified.forEach(change => {
        ...
        if (comparison.modalsChanged.length > 0) {
            ...
            comparison.modalsChanged.forEach(change => {
                console.log(`   - ${change.modalId}: ${change.change}`);
            });
        }

        // Summary
        const totalChanges = comparison.usersAdded.length + 
